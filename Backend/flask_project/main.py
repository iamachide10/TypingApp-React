...
from models import User,GeneralSettings,ThemeSettings,db,CustomPassageSettings,History
...
def register():
    ...
    if existing_user and  existing_user.is_verified:
        ...
        try:
            ...
            db.session.commit()
            token = secrets.token_urlsafe(32)
            hashed_token = set_password(token)
            expired_time = datetime.utcnow() + timedelta(minutes=15)
            reset_token = ResetToken(user_id=new_user.id,token=hashed_token,expired_at = expired_time)
            db.session.add(reset_token)
            db.session.commit()
            subject= "Please verify your email"
...
def log_user():
    ...
    if user_login.check_password(user_password):
        if user_login.is_verified:
            ...
            try:
                token = secrets.token_urlsafe(32)
                strange_token = set_password(token)
                expiration_time = datetime.utcnow() + timedelta(minutes=15)
                reset_token = ResetToken(user_id=user_login.id,token=strange_token,expired_at=expiration_time)
                db.session.add(reset_token)
                db.session.commit()
                subject = "Verify your email"
                body = f"Please click this link to resend verification email.\n\n{request.host_url}resend-verification?token={token}"
                status = send_emails(user_login.email, subject, body)    
                if status == 202:
                    return jsonify({"message": "A resend verification link has been sent to your inbox"})
                else:
...
def verify_new():
    ...
    for reset in bring_all:
        if check_password(token,reset.token):
            existence = reset
            break
...
def new_verification():
    ...
    if check_exist.expires_at < datetime.utcnow():
        return jsonify({"message":"Token has expired"})
    if check_exist.used:
...
def history():
    ...
    stopped_time = data.get("stopped_time")
    finish = data.get("completed")
    if words_per_min is None or accuracy is None or stopped_time is None or finish is None:       
         return jsonify({"message":"Must include all necessary details"})
    try:
        new_data = History(user_id=check.id,wpm=words_per_min,accuracy=accuracy,stopped_time=stopped_time,completed=finish)
        ...
        return jsonify({"message":"An error occurred when trying to save user scores"}),500
...
def leaderboard():
    ...
    verify = User.query.filter_by(id=retrieve_id).first()
    if not verify:
        return jsonify({"message":"User not found"})
